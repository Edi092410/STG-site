...
//   const [selectedChips, setSelectedChips] = useState([]);
  ...
  const saveSelectedChips = () => {
    ...
    if(selectedChipNames.length !== 0){
        ...
        // localStorage.setItem('selectedProgram', JSON.stringify(selectedChipNames));
    } else {
        SetSelected(false);
        console.log("false");
        notify();
  ...
  return (
    <Main head={'Өөрт тохирсон системийн тусламж, гарын авлага, сервис үйлчилгээ, Ня-Бо санхүүгийн контентийг сонгож аваарай'}>
          ...
          <Box>
            <div className='w-full h-full flex flex-col items-center justify-center px-[20%] py-[60px]'>
              <div className='font-bold text-[#032D60] text-center '>
                Та аль програмын хэрэглэгч вэ? Дор хаяж нэг бүтээгдэхүүн сонгох хэрэгтэй.
              </div>
              <div className=' grid grid-cols-2 gap-[30px] mt-4 mb-8'>
                {data.map((item, index) => (
                  <div
                    key={index}
                    className={`w-[120px] h-[60px] flex items-center justify-center rounded-lg shadow-[0px_4px_4px_0px_rgba(0,0,0,0.10)] cursor-pointer ${selectedChips[index] ? ' transition duration-300 bg-slate-200' : ' transition duration-300 hover:scale-110'} ${selected === false && " border border-red-500"} `} 
                    onClick={() => toggleChip(index)}
                  >
                    <img src={item.img} alt={index} width={item.width} height={item.height} />
                  </div>
                ))}
              </div>
              <div
